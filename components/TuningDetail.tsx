...
const TuningDetail: React.FC<TuningDetailProps> = ({
  tuning,
  onBack,
  onUrlChange,
}) => {
  ...
  const playScale = () => {
    audioService.stopAllSounds();
    const frequencies = tuning.scale.ratios.map(
      (r) => tuning.baseFrequency * r
    );
  ...
  const playChord = (intervals: number[]) => {
    ...
    const frequencies = intervals.map(
      (i) => tuning.baseFrequency * tuning.scale.ratios[i]
    );
  ...
  const playThePiece = () => {
    ...
    if (tuning.piece.audioUrl) {
      ...
      audioService.playPiece(
        tuning.baseFrequency,
        tuning.scale.ratios,
        tuning.piece.noteSequence
      );
    }
  };
  ...
  return (
    <>
      ...
      >
        ...
        >
          <h1 className="text-3xl md:text-5xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-cyan-300 to-purple-500">
            {tuning.name}
          </h1>
          <button
            onClick={handleBack}
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <motion.div className="space-y-4" variants={itemVariants}>
            ...
            <div className="flex flex-col space-y-3">
              <button
                onClick={playScale}
                className="flex items-center justify-between w-full text-left p-3 bg-gray-800/50 hover:bg-gray-700/70 rounded-md transition-all duration-200"
              >
              ...
              <div className="pl-4 border-l-2 border-gray-700 space-y-3">
                ...
                {tuning.chords.map((chord, index) => (
                  <button
                    key={index}
                    onClick={() => playChord(chord.intervals)}
                    className="flex items-center justify-between w-full text-left p-3 bg-gray-800/50 hover:bg-gray-700/70 rounded-md transition-all duration-200"
                  >
              ...
              <button
                onClick={() => audioService.stopAllSounds()}
                className="flex items-center justify-between w-full text-left p-3 bg-red-900/50 hover:bg-red-800/70 rounded-md transition-all duration-200"
              >
